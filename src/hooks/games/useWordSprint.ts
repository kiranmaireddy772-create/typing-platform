...
import { getRandomWord, GAME_WORDS } from "@/data/games/words";
import {
  saveWordSprintScore,
  getWordSprintBest,
} from "@/lib/games/gameStorage";
import {
  calculateWPM,
  calculateAccuracy,
} from "@/lib/typing/typingCalculations";
...
export function useWordSprint(initialDuration: 30 | 60 = 30) {
  ...
  const endGame = useCallback(
    (
      ...
      const { isNewBest: newBest } = saveWordSprintScore(
        finalScore,
        finalWpm,
        finalAccuracy
      );
  ...
  const handleInputChange = useCallback(
    (newVal: string) => {
      ...
      if (newVal.trim() === currentWord) {
        ...
        const newScore = calculateWordSprintScore(
          newWordsTyped,
          wpm
        );
        ...
        // Reset tracking if 70% of medium words have been used to cycle safely
        if (usedWordsRef.current.size >= Math.floor(GAME_WORDS.medium.length * 0.7)) {
          usedWordsRef.current.clear();
        }

        const nextWord = getRandomWord("medium", usedWordsRef.current);
        usedWordsRef.current.add(nextWord);
