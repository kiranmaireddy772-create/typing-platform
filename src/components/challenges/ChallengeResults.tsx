...
import { Flame, Sparkles, RefreshCw, ArrowLeft } from "lucide-react";
...
export function ChallengeResults({
  ...
  return (
    <div className="rounded-3xl border border-amber-500/30 bg-slate-900/95 p-8 sm:p-12 text-center shadow-2xl space-y-6 backdrop-blur-xl animate-fade-in max-w-xl mx-auto">
      ...
      {/* Score Box */}
      <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-6 max-w-sm mx-auto">
        <div className="text-xs uppercase font-bold text-amber-400 tracking-wider">Final Score</div>
        <div className="mt-1 text-5xl font-extrabold font-mono text-white">
          {score}
        </div>
        {bestScore !== undefined && bestScore !== null && (
          <div className="mt-2 text-xs text-slate-400 font-mono">
            Best Daily Score: <strong className="text-amber-400">{Math.max(score, bestScore)}</strong>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 my-6">
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-950 p-3">
          ...
          <div className="text-lg font-bold font-mono text-rose-400">{errors}</div>
        </div>

        <div className="rounded-xl border border-slate-800 bg-slate-950 p-3">
          <div className="text-[10px] uppercase font-bold text-slate-400">Streak</div>
          <div className="text-lg font-bold font-mono text-amber-400">{streak} Days</div>
        </div>
      </div>

      {/* Retries notice */}
      <p className="text-xs text-slate-400 italic">
        You may retry today&apos;s challenge to improve your score. Retries will not double-increment your streak.
      </p>
